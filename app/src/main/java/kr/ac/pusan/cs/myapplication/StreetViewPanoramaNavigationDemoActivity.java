...
public class StreetViewPanoramaNavigationDemoActivity extends AppCompatActivity {
    ...
    // Sangbong, korea
    private static final LatLng SANGBONG = new LatLng(37.59523, 127.08600);

    // Busan, korea
    private static final LatLng BUSAN = new LatLng(35.231657, 129.084337);

    // Tokyo, japan
    private static final LatLng TOKYO = new LatLng(35.685506, 139.776822);

    private static final LatLng MONSERRAT = new LatLng(-34.604399, -58.381615);

    // LatLng with no panorama
    private static final LatLng INVALID = new LatLng(-45.125783, 151.276417);

    private static LatLng LOCATION = new LatLng(37.769263, -122.450727);
    /**
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        int locationKey = intent.getExtras().getInt("key");
        locationKey %= 7;
        switch (locationKey) {
            case 0:
                LOCATION = BUSAN;
     ...
     */
    public void onGoToSantorini() {
        if (!checkReady()) {
            return;
        }
        mStreetViewPanorama.setPosition(SANGBONG);
    }

    /**
     * Called when the Animate To Invalid button is clicked.
     */
    public void onGoToInvalid() {
        if (!checkReady()) {
            return;
        }
